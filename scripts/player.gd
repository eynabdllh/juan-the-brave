...
func _ready():
	...
	# Ensure this node is in the 'player' group for death tracking
	if not is_in_group("player"):
		add_to_group("player")
		# Prepare speed trail (created lazily on first use)
	
	# Initialize health in global system
	if has_node("/root/global"):
		var g = get_node("/root/global")
		if g.has_method("set_player_health"):
			g.set_player_health(health)
...
func current_camera():
	if not use_local_camera:
		...
		if has_node("doorside1_camera"): $doorside1_camera.enabled = false
		if has_node("doorside2_camera"): $doorside2_camera.enabled = false
		if has_node("myth_camera"): $myth_camera.enabled = false
		return
	if global.current_scene == "world":
		...
		if has_node("doorside2_camera"): $doorside2_camera.enabled = false
		if has_node("myth_camera"): $myth_camera.enabled = false
	elif global.current_scene == "map_3":
		if has_node("world_camera"): $world_camera.enabled = false
		if has_node("doorside_camera"): $doorside_camera.enabled = false
		if has_node("cemetery_camera"): $cemetery_camera.enabled = false
		if has_node("doorside1_camera"): $doorside1_camera.enabled = false
		if has_node("doorside2_camera"): $doorside2_camera.enabled = false
		if has_node("myth_camera"): $myth_camera.enabled = true
	elif global.current_scene == "door_side_1":
		if has_node("world_camera"): $world_camera.enabled = false
		if has_node("doorside_camera"): $doorside_camera.enabled = false
		if has_node("cemetery_camera"): $cemetery_camera.enabled = false
		if has_node("doorside1_camera"): $doorside1_camera.enabled = true
		if has_node("doorside2_camera"): $doorside2_camera.enabled = false
		if has_node("myth_camera"): $myth_camera.enabled = false
	elif global.current_scene == "door_side_2":
		if has_node("world_camera"): $world_camera.enabled = false
		if has_node("doorside_camera"): $doorside_camera.enabled = false
		if has_node("cemetery_camera"): $cemetery_camera.enabled = false
		if has_node("doorside1_camera"): $doorside1_camera.enabled = false
		if has_node("doorside2_camera"): $doorside2_camera.enabled = true
		if has_node("myth_camera"): $myth_camera.enabled = false
...
# --- Death flow -> Game Over when all players are dead ---
func die() -> void:
	# Disable local processing/controls
	set_process_input(false)
	set_physics_process(false)
	velocity = Vector2.ZERO

	# Optional: play a death anim if available
	...
	# Determine if this is the last living player before removal
	var others_alive := 0
	var player_list := get_tree().get_nodes_in_group("player")
	print("[die] players in group 'player': ", player_list.size())
	for n in player_list:
		...
		if is_instance_valid(n):
			var alive := false
			if n.has_method("get"):
				var v = n.get("player_alive")
				alive = (v == true)
			elif "player_alive" in n:
				alive = n.player_alive
			if alive:
				others_alive += 1

	var should_game_over := (others_alive == 0)
	print("[die] others_alive=", others_alive, " should_game_over=", should_game_over)

	# If this is the last player, show Game Over overlay on top of the current scene
	if should_game_over:
		await get_tree().create_timer(0.2).timeout
		if ResourceLoader.exists("res://scenes/game_over.tscn"):
			var existing := get_tree().root.get_node_or_null("GameOverLayer")
			if existing == null:
				var ps: PackedScene = load("res://scenes/game_over.tscn")
				if ps:
					var ui := ps.instantiate()
					var layer := CanvasLayer.new()
					layer.name = "GameOverLayer"
					layer.layer = 100
					# Add overlay and pause
					get_tree().root.add_child(layer)
					layer.add_child(ui)
					get_tree().paused = true
					print("[die] Game Over overlay shown")
				else:
					print("[die] Failed to load game_over.tscn")
			else:
				print("[die] Game Over overlay already present")
		else:
			print("[die] game_over.tscn missing; quitting")
			get_tree().quit()
		return

	# Otherwise, remove only this player after a brief delay
