...
func _on_mp_host_pressed() -> void:
	print("[MainMenu] Host pressed")
	if typeof(network) == TYPE_NIL:
		push_warning("Network autoload missing. Check Project Settings -> Autoload for 'network'.")
		return
	network.create_server()
	get_tree().change_scene_to_file("res://scenes/lobby.tscn")

func _on_mp_join_pressed() -> void:
	print("[MainMenu] Join pressed")
	get_tree().change_scene_to_file("res://scenes/lobby_browser.tscn")

func _on_mp_local_pressed() -> void:
	# Fallback: treat Local as Host for now
	print("[MainMenu] Local pressed -> hosting locally")
	if typeof(network) == TYPE_NIL:
		push_warning("Network autoload missing. Check Project Settings -> Autoload for 'network'.")
		return
	network.create_server()
